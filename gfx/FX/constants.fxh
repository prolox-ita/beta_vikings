...
static const float GB_CAM_MAX_FILLING_CLAMP = 0.83f; // 0 to 1 value for clamping the fill when camera is at max distance
static const float GB_THRESHOLD = 0.13f; // interpolation time
static const float GB_THRESHOLD2 = 0.25f; // interpolation time
//static const float3 GB_OUTLINE_COLOR = float3( 0.0f, 0.0f, 0.0f );
static const float GB_OUTLINE_CUTOFF_SEA = 0.7f; // Magic number to balance cutoff on edges without neighbor (over Sea)
